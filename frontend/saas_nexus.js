(function(){
  ...
  function renderUserCards(){
    ...
    const ctr = user.controls || {};
    const riskLocked = !!ctr.risk_day_locked;
    const riskLockBanner = riskLocked
      ? '<div class="nx-status error" style="margin-top:14px"><b>Trading paused for today (IST)</b><div class="nx-item-sub" style="margin-top:8px">Desk journal PnL reached your daily loss limit. Open journal rows were closed at desk level (flatten does not send exchange orders). Close any live broker positions yourself if needed. New signals and auto-execution stay off until the next IST calendar day. Today PnL '+fmtMoney(ctr.today_journal_pnl||0)+' · cap '+fmtMoney(ctr.daily_loss_limit||0)+'.</div></div>'
      : '';
    const statusRiskRow = riskLocked
      ? '<div class="nx-item" style="margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,.08)"><div class="nx-item-title">Risk lock</div><div class="nx-item-sub">No new signals or auto-exec until midnight IST. Clears automatically on the next IST day.</div></div>'
      : '';
    ...
    const overviewCard = '<div class="nx-card"><div class="nx-card-head"><div><div class="nx-card-title">Overview</div><div class="nx-card-sub">Wallet, signals, and performance snapshot</div></div><span class="nx-badge '+(riskLocked?'bad':'cool')+'">'+escapeHtml((user.subscription||{}).plan_code || 'No Plan')+'</span></div><div class="nx-card-body"><div class="nx-metric-grid"><div class="nx-metric"><div class="nx-metric-k">Balance</div><div class="nx-metric-v">'+fmtMoney((((user.wallet||{}).balance)||0))+'</div></div><div class="nx-metric"><div class="nx-metric-k">Unread</div><div class="nx-metric-v">'+fmtNum(metrics.signals_unread||0)+'</div></div><div class="nx-metric"><div class="nx-metric-k">Closed Trades</div><div class="nx-metric-v">'+fmtNum((perf.summary||{}).closed_trades||0)+'</div></div><div class="nx-metric"><div class="nx-metric-k">Total PnL</div><div class="nx-metric-v">'+fmtMoney((perf.summary||{}).total_pnl||0)+'</div></div></div><div class="nx-inline-note" style="margin-top:14px">Controls: daily loss '+fmtMoney((ctr.daily_loss_limit)||0)+' · max trades '+fmtNum((ctr.max_trades_per_day)||0)+' · profit share '+fmtNum((ctr.profit_share_pct)||0)+'%'+(riskLocked?'':' · today journal PnL '+fmtMoney(ctr.today_journal_pnl||0))+'</div>'+riskLockBanner+'</div></div>';
    const statusStrip = '<div class="nx-card"><div class="nx-card-head"><div><div class="nx-card-title">Workspace status</div><div class="nx-card-sub">Live access posture for your desk, broker, and payments.</div></div></div><div class="nx-card-body"><div class="nx-workspace-mini-grid"><div class="nx-item"><div class="nx-item-title">Subscription</div><div class="nx-item-sub">'+escapeHtml((user.subscription||{}).plan_code || 'No active plan')+' · '+escapeHtml((user.subscription||{}).status || 'NONE')+'</div></div><div class="nx-item"><div class="nx-item-title">Broker</div><div class="nx-item-sub">'+escapeHtml(broker.broker_name || 'Paper Router')+' · '+escapeHtml(broker.status || 'Idle')+'</div></div><div class="nx-item"><div class="nx-item-title">Notifications</div><div class="nx-item-sub">'+escapeHtml((notifications.email ? 'Email ' : '') + (notifications.telegram ? 'Telegram ' : '') + (notifications.whatsapp ? 'WhatsApp ' : '') || 'All disabled')+'</div></div>'+statusRiskRow+'</div></div></div>';
    ...
    const controlsCard = '<div class="nx-card"><div class="nx-card-head"><div><div class="nx-card-title">Risk & Automation</div><div class="nx-card-sub">Daily limits, auto-exec, and alerts. Strategy on/off is under <b>Signals</b>.</div></div></div><div class="nx-card-body"><div class="nx-toggle-grid"><label class="nx-check"><input id="nx-user-auto-execute" type="checkbox"'+checkedAttr(!!(ctr.auto_execute))+'><span>Auto execute</span></label><div class="nx-pay-chip"><span>Broker</span><strong>'+escapeHtml(broker.broker_name || 'Paper Router')+'</strong></div><div class="nx-pay-chip"><span>Desk mode</span><strong>'+(brokerDeskLive ? 'Live (exchange)' : 'Safe / Paper')+'</strong></div><div class="nx-pay-chip"><span>Broker status</span><strong>'+escapeHtml(broker.status || 'Idle')+'</strong></div></div><div class="nx-form-grid-3" style="margin-top:12px"><label class="nx-form-label">Daily loss (₹)<input id="nx-user-daily-loss" class="nx-input" placeholder="2500" value="'+escapeHtml(String(ctr.daily_loss_limit || 0))+'"></label><label class="nx-form-label">Max trades / day<input id="nx-user-max-trades" class="nx-input" placeholder="6" value="'+escapeHtml(String(ctr.max_trades_per_day || 0))+'"></label><label class="nx-form-label">Max open signals<input id="nx-user-max-open" class="nx-input" placeholder="3" value="'+escapeHtml(String(ctr.max_open_signals || 0))+'"></label></div><div class="nx-inline-note" style="margin-top:12px">Sum of <b>desk journal</b> PnL for trades with <b>open date today (IST)</b> is compared to daily loss. If it hits the cap, open journal rows are flattened at desk level (no automatic Kite exits) and <b>no new signals</b> for you until the next IST day.</div>'+(riskLocked?'<div class="nx-status warn" style="margin-top:12px"><b>Day risk lock on</b> — routing stays off until midnight IST.</div>':'')+'<div class="nx-actions" style="margin-top:12px"><button class="nx-btn nx-btn-primary" onclick="nxSaveNotifications()">Save Preferences</button></div></div></div>';
